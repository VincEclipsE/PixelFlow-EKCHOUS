...
- **No Stam fluid solver.** Iterating to convergence with float reductions is vendor-sensitive. Atmosphere = decay-particle field diffusion (`decay_system §4.11`); vascular = 1D Gauss-Seidel with fixed-point pressure.
...
- **No float reductions.** COM, total_mass, angular_inertia, etc. accumulate in fixed-point integer accumulators via Kogge-Stone prefix sum; float is read out only at use-time.
